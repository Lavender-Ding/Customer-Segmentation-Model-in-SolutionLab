function [W,M,V,L,E,Lb] = EM_GM_fast(X,k,ltol,maxiter,pflag,Init)
...
while (abs(100*(Ln-Lo)/Lo)>ltol) & (niter<=maxiter),
    E1 = Expectation(X,k,W,M,V);     % E-step
    [W,M,V] = Maximization(X,k,E1);  % M-step
    Lo = Ln;
...
scatter3(X(:,1),X(:,2),X(:,3),[],Lb./k,'.');
% scatter(X(:,1),X(:,2),[],Lb./k,'.');
figure;
